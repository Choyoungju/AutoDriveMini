...
class Game:
    ...
    def reset(self):
        ...
        self.block[0]["col"] = int(random.uniform(self.road_left, self.road_right + 1))
        self.block[0]["row"] = 0
        self.block[1]["col"] = int(random.uniform(self.road_left, self.road_right + 1))
        self.block[1]["row"] = 0
    ...
    def update_car(self, move):
        ...
        self.car["col"] = max(int(self.road_left), int(self.car["col"]) + move)
        # 자동차의 위치가 도로의 우측을 넘지 않도록 합니다.: min(max, screen_width) < screen_width
        self.car["col"] = min(int(self.car["col"]), int(self.road_right))
    ...
    def update_block(self):
        ...
        if self.block[1]["row"] > 0:
            ...
            self.block[1]["col"] = random.randrange(int(self.road_left), int( self.road_right + 1))
